...
class LivConstatPoulard extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $this->cycleActifs = Cycle::where('actif', 1)->where('id_type_poulet', 8)->get();
    ...
    {
        $constats = DB::table('constat_poulards')
            ->join('cycles', 'cycles.id', 'constat_poulards.id_cycle')
            ...
            //->join('users', 'users.id', 'constat_poulards.id_utilisateur')
            ->select('constat_poulards.*', 'type_poulets.type', 'cycles.description', 'batiments.nom', 'sites.site', 'sites.adresse')
            ->paginate(20);
        ...
        return view('livewire.liv-constat-poulard', [
            'constats' => $constats,
            'sites' => $sites,
            'batiments' => $batiments,
    ...
    */
    public function createSortieConstant()
    {
        ...
        try{
            ...
            session()->flash('message', 'Constat poulard bien enregistré!');
    ...
    */
    ...
    {
        $constat = ConstatPoulard::findOrFail($id);
        $this->constat_id = $id;
        $this->nb = $constat->nb;
        $this->new_nb = '';
    ...
    public function updateConstat()
    {
        ...
        try{
            ...
            $constat = ConstatPoulard::findOrFail($this->constat_id);
    ...
    {
        $this->recordToDelete = ConstatPoulard::findOrFail($id);
    }
    ...
    */
    ...
    public function saveSortieAndDetailForNewClient()
    {
        ...
        $constat = ConstatPoulard::where('id', $this->id_dernier_constat)->first();
        ...
        if($constat !=null)
        {
            ...
            DB::beginTransaction();
                    try{
                        ...
                        $this->isLoading = false;
                        $this->notification = true;
                        session()->flash('message', 'Sortie poulard bien enregistré!');
                        DB::commit();
                        $this->resetPage();
                    }catch(\Exception $e){
                        DB::rollback();
